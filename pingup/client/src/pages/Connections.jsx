...
import { MessageSquare, User, UserCheck, UserPlus, UserRoundPen, Users } from 'lucide-react'
...
const Connections = () => {
  ...
  const handleUnfollow = async (userId) => {
    try {
      const { data } = api.post('/api/user/unfollow', {id: userId}, {
  ...
  const acceptConnection = async (userId) => {
    try {
      const { data } = api.post('/api/user/accept', {id: userId}, {
        headers: {Authorization: `Bearer ${await getToken()}`}
      })
      if(data.success) {
        toast.success(data.message)
  ...
  <div className='min-h-screen bg-slate-50'>
    ...
    {/* Tabs */}
  <div className='inline-flex flex-wrap items-center px-3 py-1 text-sm rounded-md p-1 bg-white shadow-sm'>
    {
      dataArray.map((tab)=>(
        <button onClick={()=> setCurrentTab(tab.label)} key={tab.label} className={`flex cursor-pointer items-center px-3 py-1 text-sm rounded-md transition-colors 
        ${currentTab === tab.label ? 'bg-white font-medium text-black' : 'text-gray-500 hover:text-black'}`} >
            <tab.icon className='w-4 h-4'/>
            <span className='ml-1'>{tab.label}</span>
            {
              tab.count !== undefined && (
                <span className='ml-2 text-xs text-gray-700 px-2 py-0.5 rounded-full'>{tab.count}</span>
              )
            }
        </button>
      ))
          ...
          <div className='flex max-sm:flex-col gap-2 mt-4'>
            {
              ...
              hover:from-indigo-600 hover:to-purple-700 active:slate-95 transition text-white cursor-pointer'>
                View Profile
              </button>
            }
            {
              currentTab === 'Following' && (
                ...
                text-black active:slate-95 transition cursor-pointer' >
                  Unfollow
                </button>
              )
            }
